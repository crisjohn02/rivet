...
**No evidence-based exclusion for TypeScript in v0.1 (T43).** Both kinds of evidence are PHP's. The form table is derived from the forms the PHP extractor records, and TypeScript routinely references a function or a `const` through a qualifier (`Outer.f()`, a namespace import's `util.format()`, `Color.Red`), names a class through `new`, and puts a value in a type position (`typeof x`), so the table would drop real references. The receiver rule assumes nominal classes, and under TypeScript's structural typing a receiver typed `Foo` may hold any shape-compatible object. So for a target or a use in a TypeScript file neither rule applies, and `by_exclusion` stays 0. A use also matches a target by name only within the target's language, compared by that language's rule (PHP folds ASCII case for class-like, function, and method names; TypeScript compares every name exactly): a TypeScript use never names a PHP declaration, nor the reverse.
