...
enum TokenType {
    ...
    ERROR_RECOVERY,
    CALL_SIGNATURE_AUTOMATIC_SEMICOLON,
    TYPE_MEMBER_NAME,
};
...
static bool scan_automatic_semicolon(TSLexer *lexer, const bool *valid_symbols, bool *scanned_comment) {
    ...
    switch (lexer->lookahead) {
        ...
        case '{':
            ...
            // rivet patch 1: between the members of an object type or interface
            // body, a `<` on a new line starts a generic call signature, as
            // TypeScript reads it (a type never takes type arguments across a
            // line break). Everywhere else, and right after a member's name,
            // where the `<` opens a method signature's type parameters, `<`
            // continues the previous line as before.
        case '<':
            if (valid_symbols[CALL_SIGNATURE_AUTOMATIC_SEMICOLON] && !valid_symbols[TYPE_MEMBER_NAME]) {
                lexer->result_symbol = CALL_SIGNATURE_AUTOMATIC_SEMICOLON;
                return true;
            }
            return false;
