...
module.exports = function defineGrammar(dialect) {
  return grammar(JavaScript, {
    ...
    externals: ($, previous) => previous.concat([
      ...
      $.__error_recovery,
      // rivet patch 1: see RIVET-PATCHES.md and common/scanner.h.
      $._call_signature_automatic_semicolon,
      $.__type_member_name,
    ]),
    ...
    rules: {
      ...
      export_statement: ($, previous) => choice(
        ...
        // rivet patch 2: `export type * from` and `export type * as ns from`
        // (TypeScript 5.0), shaped like `export * from` plus a `type` token.
        seq(
          'export',
          'type',
      ...
      method_signature: $ => seq(
        ...
        field('name', $._property_name),
        optional('?'),
        // rivet patch 1: never produced; its validity tells the scanner that
        // a `<` here would open this method signature's type parameters.
        optional($.__type_member_name),
        $._call_signature,
      ),
      ...
      object_type: $ => seq(
        ...
        optional(seq(
          optional(choice(',', ';')),
          sepBy1(
            // rivet patch 1: `_call_signature_automatic_semicolon` is the
            // automatic semicolon before a `<` that starts a generic call
            // signature on a new line.
            choice(',', $._semicolon, $._call_signature_automatic_semicolon),
            choice(
              $.export_statement,
              $.property_signature,
              $.call_signature,
