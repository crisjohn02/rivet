...
//! the focus names to be exactly the gold's. T44's and T45's `[[binding]]`
//! entries need the resolver, so their harnesses are
//! `rivet-index/tests/typescript_gold.rs` (extractor and resolver) and the
//! CLI's `typescript_index.rs` (end to end); this file checks only that they
//! are well formed. `tests/gold/check_gold.py` checks only the recorded spans
//! and names these tests (and, for T43, T44, and T45, the CLI's
//! `typescript_index.rs`) as the owners of the comparison.
