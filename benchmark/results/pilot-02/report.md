# Rivet Benchmark Report — pilot-02
...
| Mean input-token reduction | 16.1% (C/B = 0.839 over 5 paired tasks; pilot point estimate, no interval) |
| Task success difference (C − B) | 0.0 percentage points over 5 paired tasks (pilot) |
| End-to-end wall-time change | +2.5% (pilot) |
...
| Study manifest, artifact checksums | `benchmark/studies/pilot-02/study.toml` sha256 `ea4eb57b9226096f`; runs.csv sha256 `536320bb32560827` |
...
| Run ordering / randomization seed | seed 20260924; arm order randomized within each task/trial block |
...
| Study run/spend cap and actual spend | $9.00 cap; $3.80 reported (Claude Code estimate, not billing); 0 attempts with unknown cost |
...
|---|---|---|---|---|---|
| Mean total input tokens / run | — | 67,406 | 56,552 | Ratio: 0.839; reduction: 16.1% | not computed (pilot) |
| Task success rate | — | 100% | 100% | Difference: 0.0 percentage points | not computed (pilot) |
| Mean end-to-end wall time / run | — | 23.3 s | 23.8 s | Change: +2.5% | not computed (pilot) |
| Mean tool calls / run | — | 6.5 | 4.2 | Change: -35.1% | not computed (pilot) |
| Mean output tokens / run | — | 1,600 | 1,441 | Change: -9.9% | not computed (pilot) |

| Confirmatory gate | Proposed preregistered requirement | Estimate / bound | Outcome |
|---|---|---|---|
| Efficiency point estimate | C/B ≤ 0.70 | C/B = 0.839 (pilot) | NOT EVALUATED (pilot) |
...
|---|---|---|---|---|---|---|---|
| p1 | timesheet / locate | 100% | 100% | 28,781 | 26,160 | 9.1% | -11.4% |
| p2 | timesheet / callers | 100% | 100% | 38,700 | 35,618 | 8.0% | +16.3% |
| p3 | timesheet / tests | 100% | 100% | 160,585 | 136,620 | 14.9% | +14.9% |
| p4 | timesheet / trace | 100% | 100% | 76,407 | 55,641 | 27.2% | -20.6% |
| p5 | timesheet / dependencies | 100% | 100% | 32,554 | 28,719 | 11.8% | +10.2% |
...
| symbol / refs / context invocations | 5 / 13 / 6 (other: 0) | Command traces |
| Errors by exit code | none | Heuristic attribution from harness tool results |
| Text-tool fallbacks | 4 over 15 runs | Heuristic next-two-tool-calls rule, TASK-FORMAT.md |
