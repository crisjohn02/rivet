...
| Field | Value |
|---|---|
| Last completed task | PF2 (`--no-refresh` speed and cached diagnostic detail: the cached report counts with `COUNT(*)` and reads file statuses without source, so on Hono a cached `symbol`/`refs`/`context` takes 128-138 ms against 170-182 ms refreshed (was 241-248 ms); a refresh persists each failed file's parser diagnostic in the existing `diagnostics` table, written only when it changes, and cached and metadata-mode reports return it (size, binary, and encoding skips return the refresh's own detail); no schema or format-version change, a pre-PF2 store falls back to the generic detail; breakdown of a refreshed query in `tests/real/RESULTS.md` "PF2 addendum"); merged beside it, T44a (directory-only relative specifiers: `.`, `..`, any specifier whose last segment is `.` or `..`, and any ending in `/` are relative and name only the directory's `index.ts`, `index.tsx`, `index.d.ts`, never a file named after the directory; resolver `php-rules-v4;ts-rules-v3`; authored case aa, `src/pick/dot.ts`; Hono bindings 9285 to 9729, all 444 gained targeting a directory index, none lost, as a dated addendum in `tests/real/RESULTS.md` and `manifest.toml`) |
| Next task | GR1: vendor a minimally patched tree-sitter-typescript 0.23.2 (user-approved 2026-09-24) so newline-separated generic call signatures and `export type * from` parse; then T47 via the count_tokens API (user-approved; needs ANTHROPIC_API_KEY). Follow-ups from PF2: a query decodes every stored use (~100 ms on Hono); content refreshes reparse every failed file. Parked: T33c, T36a-T36c; TypeScript class-name receivers (`SurveyService.create()`, `Status.Active`) |
| Active task / partial progress | None |
| Blocker | None known |
| Last checks | Merged T44a+PF2 tree: fmt, clippy -D warnings, `cargo test --workspace` (822 passed), check_gold.py, test_runner.py (90), test_check_real.py (33) all pass; before the merge, PF2: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo test --workspace` (818 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 228 TypeScript entries span-verified), `benchmark/runner/test_runner.py` (90 tests), `tests/real/test_check_real.py` (33 tests) all pass; `cargo build --release && tests/real/fetch.sh && python3 tests/real/check_real.py --only determinism,incremental` PASS (0 failed): determinism byte-identical on 4 trees, full versus `--force` identical after every step (Hono 9904, PHP 1620, TS 2344, mixed 8436 queries per store). heldout-01 (T49): outcome `inconclusive`.; and T44a: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo clippy -p rivet-languages --no-default-features --all-targets -- -D warnings`, `cargo test --workspace` (814 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 240 TypeScript entries span-verified), `benchmark/runner/test_runner.py` (90 tests), `tests/real/test_check_real.py` (33 tests) all pass; `tests/real/check_real.py --only coverage` PASS (Hono 9729 bindings: 8650 exact, 1079 scoped). PHP goldens changed only in the snapshot digest (resolver fingerprint). T46's full run is recorded in `tests/real/RESULTS.md`. heldout-01 (T49): outcome `inconclusive`. |
| Decisions to carry forward | PHP first; sequential implementation; JSON before human formatting; no new commands. 2026-09-24: the user started TypeScript (T41+) now, overriding Gate G's deferral. T42 TypeScript definition rules (type aliases, generics, locals, accessors, overloads, string-named ambient modules, spans), T43 use/scope/import rules, and T44 binding rules are in ADDING-A-LANGUAGE and the adapter README. T43: TypeScript is indexed; no evidence-based exclusion for TypeScript (spec §11.5); a use matches a target by name only within one language. T44: TypeScript bindings are `exact` only for direct relative imports, namespace-import members (a lexical module-qualified name, not receiver access), and same-file module-local declarations; a module candidate that is in the snapshot but not indexed still counts toward ambiguity; globals and ambient module bodies bind nothing past themselves. T45: a TypeScript receiver binds only through a `this`, annotation, or `const` `new` hint, `scoped`, to the receiver class's own member on the matching static/instance side; the class name is resolved where it is written; no inheritance or interface-to-implementation mapping; TypeScript records no receiver classes (no exclusion). T44a: a directory-only relative specifier names only the directory's index files; the repository root counts as a directory (`..` from `src/a.ts` checks root `index.ts`). T46: real-code checks compare CLI answers only (the store is read, read-only, just to enumerate symbols and bindings), on copies; Hono's source is never vendored; PHP's real-project coverage stays with the private corpus |
