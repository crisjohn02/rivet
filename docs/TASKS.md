...
| Next task | T13 |
| Active task / partial progress | T13 implemented on branch `task/T13` and locally checked (not merged): `file:line`, query normalization hardening, deterministic ambiguity pagination, not-found suggestions in `crates/rivet-index/src/query.rs`; integration tests in `crates/rivet-cli/tests/symbol_query_forms.rs`. Checks: `cargo fmt --all -- --check` clean; `cargo clippy --workspace --all-targets --all-features -- -D warnings` clean; `cargo test --workspace` pass (rivet-cli 4+7 index_json+7 symbol_json+13 symbol_query_forms, rivet-core 60, rivet-index 11, rivet-languages 1+6, rivet-parser 0+4, rivet-store 22; 0 failed). Orchestrator reconciles Last completed/Next on merge. |
| Blocker | None known |
