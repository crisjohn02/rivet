...
| Field | Value |
|---|---|
| Last completed task | T45 (TypeScript receiver hints as `scoped` bindings: `UseHint::This` gained `is_static`, `UseHint::Typed`/`NewExpr` gained `name_span` (the span of the annotation's type-name or `new` target `type` use), and the module scope gained `member_sides` (each class/interface member's static side; constructors unrecorded), `fact-schema=14`, all omitted from PHP fact JSON; `rules/ts_receivers.rs` behind `rules_for`: `this` names the enclosing named class, a typed or `new` hint the one class or interface its `type` use binds to through the T44 lexical rules in the scope where it is written (a generic type parameter hides it; alias, enum, namespace, merged declaration, package, re-export give nothing); the class's own member of that spelling on the matching side (static `this` binds static members, every other receiver instance members) binds `scoped`; no inheritance, no interface-to-implementation, getter/setter pairs unresolved, class-name receivers unresolved; no TypeScript receiver classes; resolver `php-rules-v4;ts-rules-v2` (PHP rules unchanged); T45's 8 gold entries verified by `crates/rivet-index/tests/typescript_gold.rs` and `crates/rivet-cli/tests/typescript_index.rs`); before it, T44 (TypeScript direct relative imports/exports and same-file lexical bindings, `exact`) |
| Next task | T46: exercise the pinned real TypeScript fixture and run two-language contract checks. Parked: T33c, T36a-T36c; TypeScript class-name receivers (`SurveyService.create()`, `Status.Active`) |
| Active task / partial progress | None |
| Blocker | None known |
| Last checks | T45: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo clippy -p rivet-languages --no-default-features --all-targets -- -D warnings`, `cargo test --workspace` (810 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 228 TypeScript entries span-verified, T42 93, T43 74, T44 53, T45 8, pending 0), `benchmark/runner/test_runner.py` (90 tests) all pass; PHP-only `index`/`symbol`/`refs`/`context`/`snippet` output (304 queries per binary over all 50 fixture symbols, JSON and human) is byte-identical to the pre-T45 binary apart from the snapshot digest (goldens' digests updated); the TypeScript fixture indexes 82 symbols, 138 uses, 67 bindings (51 exact, 16 scoped); `rivet snippet` sha256 908710a5ba23e4ffc3a4fe08513ceb01a9f5b1ed02227b97d0529f7ee38e5898 (unchanged). heldout-01 (T49): outcome `inconclusive`. |
| Decisions to carry forward | PHP first; sequential implementation; JSON before human formatting; no new commands. 2026-09-24: the user started TypeScript (T41+) now, overriding Gate G's deferral. T42 TypeScript definition rules (type aliases, generics, locals, accessors, overloads, string-named ambient modules, spans), T43 use/scope/import rules, and T44 binding rules are in ADDING-A-LANGUAGE and the adapter README. T43: TypeScript is indexed; no evidence-based exclusion for TypeScript (spec §11.5); a use matches a target by name only within one language. T44: TypeScript bindings are `exact` only for direct relative imports, namespace-import members (a lexical module-qualified name, not receiver access), and same-file module-local declarations; a module candidate that is in the snapshot but not indexed still counts toward ambiguity; globals and ambient module bodies bind nothing past themselves. T45: a TypeScript receiver binds only through a `this`, annotation, or `const` `new` hint, `scoped`, to the receiver class's own member on the matching static/instance side; the class name is resolved where it is written; no inheritance or interface-to-implementation mapping; TypeScript records no receiver classes (no exclusion) |
