...
| Field | Value |
|---|---|
| Last completed task | T42 (TypeScript/TSX definition adapter `crates/rivet-languages/src/typescript/`: a documented scope walk yields named definitions with kind, lexical `.` qualified name, span, name span, parent, signature, and JSDoc for both grammar variants; new additive kind `type_alias`; `ExtractedSymbol.name_span`; the nine T42 `[[undecided]]` gold entries settled per the orchestrator's decisions; `typescript_gold.rs` compares all 93 T42 entries with the extractor; `LanguageId::has_extractor` stays `false` for TypeScript, CLI output unchanged); before it, T41 (TypeScript/TSX fixture, gold, dispatch) |
| Next task | T43: TypeScript scopes, uses, and imports (settle the three T43 `[[undecided]]` use entries), then flip `LanguageId::has_extractor` for TypeScript and add its `rivet_parser` dispatch arm together; TypeScript `lookup_name` and IDs then feed refresh. Parked: T33c, T36a-T36c |
| Active task / partial progress | None |
| Blocker | None known |
| Last checks | T42: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo clippy -p rivet-languages --no-default-features --all-targets -- -D warnings` (fixed: it failed on unused fingerprint test helpers), `cargo test --workspace` (740 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 227 TypeScript entries span-self-checked; T42's 93 span-verified, their extractor comparison owned by `typescript_gold.rs`; pending T43 74, T44 52, T45 8), `benchmark/runner/test_runner.py` (90 tests) all pass; `index`/`symbol`/`refs`/`context` output on PHP plus the TypeScript fixture is byte-identical to the pre-T42 binary; `rivet snippet` sha256 908710a5ba23e4ffc3a4fe08513ceb01a9f5b1ed02227b97d0529f7ee38e5898 (unchanged). heldout-01 (T49): outcome `inconclusive`. |
| Decisions to carry forward | PHP first; sequential implementation; JSON before human formatting; no new commands. 2026-09-24: the user started TypeScript (T41+) now, overriding Gate G's deferral; `.ts`/`.tsx` files stay `unsupported_language` until T43 flips the extractor switch with uses. T42 TypeScript definition rules (type aliases, generics, locals, accessors, overloads, string-named ambient modules, spans) are in ADDING-A-LANGUAGE and the adapter README |
