...
| Blocker | None known |
| Last checks | rustc 1.98.1 (48a229cea 2026-09-01); cargo 1.98.1 (797e8a9bc 2026-08-05). T01: workspace checks, `rivet --version` prints `rivet 0.1.0`, `rivet symbol Foo` exits 1. T02 pinned `[workspace.dependencies]` to `tree-sitter = =0.27.0`, `tree-sitter-php = =0.24.2`, `tree-sitter-typescript = =0.23.2`; all three grammar crates declare `tree-sitter-language = "0.1"`, resolved to 0.1.8, matching core. `[workspace.package] rust-version = "1.90"` is the highest declared by any package in the resolved graph (`cargo metadata`; tree-sitter and tree-sitter-language), below installed 1.98.1. Builds finish for `--no-default-features`, `--no-default-features --features lang-php`, and default; no-features `rivet --version` prints `rivet 0.1.0`. `cargo fmt --all -- --check` and `cargo clippy --workspace --all-targets --all-features -- -D warnings` pass. Grammar smoke tests: 4 pass under default features, 2 pass under `-p rivet-parser --no-default-features --features lang-php`. |
...
| [ ] | T09 | Implement atomic publication of a file inventory and deterministic snapshot digest. | One transaction publishes all file rows; a deliberately failed transaction leaves the previous complete inventory intact. Digest ignores row IDs/mtime-only changes. |
| [ ] | T10 | Wire the first real `rivet index --json` path, including coverage and work counters. Establish the shared success/error transport. | Indexing the tiny fixture produces valid output; repeat indexing has correct updated/unchanged counts. Unimplemented navigation commands clearly fail. |
...
| Done | ID | Small task | Done when |
|---|---|---|---|
| [ ] | T11 | Extract PHP namespaces, classes, named functions, and methods into owned records. Detect parser error/missing nodes. | Fixture declaration names, parents, and byte spans match gold. Malformed input produces a file diagnostic and no extracted facts. |
