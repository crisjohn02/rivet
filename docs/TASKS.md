...
| Field | Value |
|---|---|
| Last completed task | T44 (TypeScript direct relative imports/exports and same-file lexical bindings: `ModuleExport` facts per module scope (`export` on a declaration, `export default` of a named declaration or an identifier, local `export { a as b }` specifiers, anonymous/expression defaults with no local name; re-exports stay `ModuleImport`s), `value_type_uses` (`new`/`instanceof`/`typeof`/class `extends` `type` uses that name values), `ambient_module` bodies, and global declarations (script files, `declare global`) kept out of `declares`, `fact-schema=13`; the TypeScript rule set in `rivet_index::resolve::rules_for` (`rules/ts_lexical.rs`, `rules/ts_namespace.rs`, shared `rules/ts_scopes.rs`): relative module lookup over the ordered candidate set, byte-exact against snapshot paths, one candidate in the snapshot and it indexed; named/aliased/default/`import type` imports, namespace-import members, and same-file bindings with shadowing and value/type spaces, all `exact`; merged declarations, re-exports, `export *`, exported imports, aliases, packages, `require`, globals and receivers unresolved; no TypeScript receiver class; resolver `php-rules-v4;ts-rules-v1` (PHP rules unchanged); spec §11.4 rule 4 and ADDING-A-LANGUAGE say namespace members are module-qualified names, not receiver access; the T41 `import type` gold entries corrected to `exact`; T44's 53 entries verified by `crates/rivet-index/tests/typescript_gold.rs` (extractor and resolver) and `crates/rivet-cli/tests/typescript_index.rs` (end to end)); before it, T43 (TypeScript/TSX uses, scopes, and imports) |
| Next task | T45: TypeScript receiver hints (`this`, `new`, annotations) as `scoped` bindings and query integration. Parked: T33c, T36a-T36c |
| Active task / partial progress | None |
| Blocker | None known |
| Last checks | T44: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo clippy -p rivet-languages --no-default-features --all-targets -- -D warnings`, `cargo test --workspace` (795 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 228 TypeScript entries span-self-checked; T42's 93, T43's 74 and T44's 53 span-verified, their comparison owned by the named Rust harnesses; pending T45 8), `benchmark/runner/test_runner.py` (90 tests) all pass; PHP-only `index`/`symbol`/`refs`/`context`/`snippet` output (354 queries over all 50 fixture symbols, JSON and human) is byte-identical to the pre-T44 binary apart from the snapshot digest and the human `index` elapsed time (goldens' digests updated); the TypeScript fixture indexes 82 symbols, 138 uses, 51 bindings (51 exact, 0 scoped); `rivet snippet` sha256 908710a5ba23e4ffc3a4fe08513ceb01a9f5b1ed02227b97d0529f7ee38e5898 (unchanged). heldout-01 (T49): outcome `inconclusive`. |
| Decisions to carry forward | PHP first; sequential implementation; JSON before human formatting; no new commands. 2026-09-24: the user started TypeScript (T41+) now, overriding Gate G's deferral. T42 TypeScript definition rules (type aliases, generics, locals, accessors, overloads, string-named ambient modules, spans), T43 use/scope/import rules, and T44 binding rules are in ADDING-A-LANGUAGE and the adapter README. T43: TypeScript is indexed; no evidence-based exclusion for TypeScript (spec §11.5); a use matches a target by name only within one language. T44: TypeScript bindings are `exact` only for direct relative imports, namespace-import members (a lexical module-qualified name, not receiver access), and same-file module-local declarations; a module candidate that is in the snapshot but not indexed still counts toward ambiguity; globals and ambient module bodies bind nothing past themselves; every receiver stays `name_match` until T45 |
