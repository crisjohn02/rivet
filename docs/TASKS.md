...
| Field | Value |
|---|---|
| Last completed task | T43 (TypeScript/TSX uses, scopes, and imports in `crates/rivet-languages/src/typescript/uses.rs`: calls, `new`, member calls with receiver text, type positions, member reads/writes and bare writes, JSX component names and `{expr}` containers, template interpolation, top-level uses; per-scope `locals` with value/type space and `declares`, so a same-name local is seen to hide an import; named/default/aliased/namespace/`require`/`import type` bindings and re-exports as `ModuleImport`s, specifiers as written; `This`/`Typed`/`NewExpr` receiver hints for T45; `LanguageId::has_extractor` on for TypeScript and TSX with the `rivet_parser` dispatch arm, `fact-schema=12`, resolver `php-rules-v4`; the resolver's language-keyed rule table binds no TypeScript use and keeps PHP rules to PHP declarations; no evidence-based exclusion for TypeScript; case-sensitive TypeScript lookup and same-language name matching; the three T43 `[[undecided]]` gold entries settled; T43's 74 entries verified by `typescript_gold.rs` and end to end by `crates/rivet-cli/tests/typescript_index.rs`); before it, T42 (TypeScript/TSX definitions) |
| Next task | T44: resolve direct relative TypeScript imports/exports (a TypeScript rule set in `rivet_index::resolve::rules_for`; correct the `import type` binding entry; it needs module export facts, which T43 does not record). Parked: T33c, T36a-T36c |
| Active task / partial progress | None |
| Blocker | None known |
| Last checks | T43: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo clippy -p rivet-languages --no-default-features --all-targets -- -D warnings`, `cargo test --workspace` (769 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 228 TypeScript entries span-self-checked; T42's 93 and T43's 74 span-verified, their comparison owned by `typescript_gold.rs` and, for T43, `typescript_index.rs`; pending T44 53, T45 8), `benchmark/runner/test_runner.py` (90 tests) all pass; PHP-only `index`/`symbol`/`refs`/`context`/`snippet` output is byte-identical to the pre-T43 binary apart from the snapshot digest (goldens' digests updated); the TypeScript fixture indexes 82 symbols, 138 uses, 0 bindings; `rivet snippet` sha256 908710a5ba23e4ffc3a4fe08513ceb01a9f5b1ed02227b97d0529f7ee38e5898 (unchanged). heldout-01 (T49): outcome `inconclusive`. |
| Decisions to carry forward | PHP first; sequential implementation; JSON before human formatting; no new commands. 2026-09-24: the user started TypeScript (T41+) now, overriding Gate G's deferral. T42 TypeScript definition rules (type aliases, generics, locals, accessors, overloads, string-named ambient modules, spans) and T43 use/scope/import rules are in ADDING-A-LANGUAGE and the adapter README. T43: TypeScript is indexed; every TypeScript use stays `name_match` until T44/T45 add rules; no evidence-based exclusion for TypeScript (spec §11.5); a use matches a target by name only within one language |
