...
| Field | Value |
|---|---|
| Last completed task | GR1 (vendored, minimally patched `tree-sitter-typescript` `0.23.2-rivet.1` at `vendor/tree-sitter-typescript/`, from tag `v0.23.2` with its MIT license, `RIVET-PATCHES.md`, committed generated C from CLI 0.24.4, and `regenerate.sh [--check]`; the build needs no CLI: between object-type members a `<` on a new line now ends the member, as TypeScript reads it, except right after a member name, and `export type * [as ns] from` parses as a type-only re-export-all; every file upstream parsed without error (fixture, Hono, 1,141 other local files) parses to an identical tree; fingerprint `ts=0.23.2-rivet.1`; Hono parse failures 8 to 0, bindings 9,729 to 14,781 with none lost or changed; dated addendum in `tests/real/RESULTS.md` and `[project.rivet_index_gr1]`); before it, PF2 and T44a (see their rows) |
| Next task | T47 via the count_tokens API (user-approved; needs ANTHROPIC_API_KEY). Follow-ups from PF2: a query decodes every stored use (~100 ms on Hono); content refreshes reparse every failed file. Parked: T33c, T36a-T36c; TypeScript class-name receivers (`SurveyService.create()`, `Status.Active`) |
| Active task / partial progress | None |
| Blocker | None known |
| Last checks | GR1: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo clippy -p rivet-languages --no-default-features --all-targets -- -D warnings`, `cargo test --workspace` (835 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 240 TypeScript entries span-verified), `benchmark/runner/test_runner.py` (90 tests), `tests/real/test_check_real.py` (33 tests) all pass; `cargo build --release && tests/real/fetch.sh && python3 tests/real/check_real.py` PASS (0 failed); a clean release build from an empty `CARGO_TARGET_DIR` with no `tree-sitter`, node, or npm on PATH works; `vendor/tree-sitter-typescript/regenerate.sh --check` reproduces the committed C and passes the grammar corpus (112 upstream + 12 GR1 cases). Goldens changed only in the snapshot digest (extractor fingerprint). heldout-01 (T49): outcome `inconclusive`. |
| Decisions to carry forward | PHP first; sequential implementation; JSON before human formatting; no new commands. 2026-09-24: the user started TypeScript (T41+) now, overriding Gate G's deferral. T42 TypeScript definition rules (type aliases, generics, locals, accessors, overloads, string-named ambient modules, spans), T43 use/scope/import rules, and T44 binding rules are in ADDING-A-LANGUAGE and the adapter README. T43: TypeScript is indexed; no evidence-based exclusion for TypeScript (spec §11.5); a use matches a target by name only within one language. T44: TypeScript bindings are `exact` only for direct relative imports, namespace-import members (a lexical module-qualified name, not receiver access), and same-file module-local declarations; a module candidate that is in the snapshot but not indexed still counts toward ambiguity; globals and ambient module bodies bind nothing past themselves. T45: a TypeScript receiver binds only through a `this`, annotation, or `const` `new` hint, `scoped`, to the receiver class's own member on the matching static/instance side; the class name is resolved where it is written; no inheritance or interface-to-implementation mapping; TypeScript records no receiver classes (no exclusion). T44a: a directory-only relative specifier names only the directory's index files; the repository root counts as a directory (`..` from `src/a.ts` checks root `index.ts`). T46: real-code checks compare CLI answers only (the store is read, read-only, just to enumerate symbols and bindings), on copies; Hono's source is never vendored; PHP's real-project coverage stays with the private corpus. GR1: the TypeScript grammar is the vendored `vendor/tree-sitter-typescript/` (`0.23.2-rivet.N`); a grammar change regenerates the committed C with `regenerate.sh`, lists itself in `RIVET-PATCHES.md`, must keep every previously error-free tree identical, and bumps the version and `EXTRACTOR_FINGERPRINT` |
...
| [x] | T29 | Suppress parent/child source overlap and count budget/overlap/limit omissions. | Target/member/parent examples contain no duplicated full bodies and estimates use the final emitted summaries. |
| [x] | T30 | Wire `context <query>` through discovery, refresh, query resolution, and JSON output. | A single invocation returns useful target/related source without requiring earlier symbol/refs commands. All context options and errors match the contract. |
...
| [x] | PF2 | Make `--no-refresh` queries cheaper than refreshed ones and keep parser detail in cached diagnostics. On Hono a cached query took 0.24–0.27 s against 0.17–0.18 s refreshed, and cached diagnostics said `stored parse error`. Found by T46. | The cached report decodes no fact row; a cached query is faster than a refreshed one on Hono; `--no-refresh` writes nothing and returns the refreshed bytes apart from `freshness` on both fixtures; cached and metadata-mode diagnostics equal the refresh's; no schema change, digests and every other output unchanged. |
...
| [x] | T32 | Complete parser resource limits, skip/diagnostic handling, and direct-target errors. | A formerly valid file loses stale facts on parse/resource failure; unrelated queries report partial coverage; direct queries fail with the specified code. |
...
| [x] | T33b | Implement snippet output and explicit/default instruction-file installation. | AGENTS.md and CLAUDE.md creation/update, both-file ambiguity, JSON, and repeat setup pass local fixtures. Shipped text matches AGENT-SNIPPET. |
...
| [x] | CV1 | Shorten the human coverage line and name its diagnostics instead of pointing at `--json`, which prompted every held-out `--json` call: `coverage <complete\|incomplete>: <indexed>/<seen> files indexed; skipped <non-zero counts>; <diagnostics>`, where `<diagnostics>` is `0 diagnostics`, one or two `<file> (<code>)` in the contract's sort order, or, above two, counts by code with no path (`50 diagnostics (50 unsupported_language)`; `first 50: ` when truncated). Review fix: naming two paths plus `+N more` made the pilot project's line about 40 tokens longer. OUTPUT-CONTRACT states the grammar. TE1 lever 5. | Every fact the old line stated is kept and the line appears under the same rule; ordinary `unsupported` files are only counted; a non-UTF-8 path stays escaped; tests cover none, one, two, three of mixed codes, count ties, 50 of one code and a truncated 60, only-`unsupported` skips, complete coverage with a diagnostic, and the error path; `--json` is byte-identical to the pre-CV1 binary (goldens in `tests/golden/cv1-head/`); the snippet hash is unchanged. |
| [x] | SN2 | Replace the managed snippet with a lean block of about 150 estimated tokens: the three navigation commands (`context` at `--tokens 1500`), the accepted name forms, automatic refresh, the `?` name-only mark, and what rivet does not see, so a short or empty result is not proof of absence. TE1 lever 1 (and lever 3 through the snippet's budget). New benchmark treatment. | `rivet snippet` and docs/AGENT-SNIPPET.md agree byte for byte; the new hash is recorded with the SN1 and pilot-01 hashes; SN1-era and pilot-01-era blocks upgrade in place with surrounding text preserved; every named command form and flag exists (`--help`) and `context --tokens 1500` is accepted; the `13-snippet` JSON golden is re-captured. |
...
| [x] | T42 | Extract TypeScript named definitions and signatures. | Named functions/classes/methods and remaining supported kinds match gold; split remaining kind families into subtasks if needed. |
| [x] | T43 | Extract TypeScript scopes, uses, and imports, including TSX expressions/component names. | Top-level/anonymous-container uses, interpolation, aliases, and shadowing match gold; literal JSX text is excluded. |
| [x] | T44 | Resolve supported direct relative imports/exports. | Local named/default bindings resolve uniquely; re-exports, package/path aliases, and ambiguous paths remain unresolved. |
| [x] | T44a | Treat directory-only relative specifiers (`.`, `..`, a last segment of `.` or `..`, a trailing `/`) as naming only the directory's `index.ts`, `index.tsx`, `index.d.ts`. | Such imports bind `exact` through a unique indexed index; never a file named after the directory; climbing above the root, empty interior segments, and ambiguous or unindexed indexes stay unresolved; resolver `php-rules-v4;ts-rules-v3`. |
| [x] | T45 | Add TypeScript receiver hints and query integration. | Supported typed/new/this hints are scoped; refs and context work through the existing generic path without language branches in core. |
| [x] | T46 | Exercise the pinned real TypeScript fixture and run two-language contract checks. | Known limitations are documented; full/incremental query bytes and coverage hold for both languages. |
| [x] | GR1 | Vendor a minimally patched `tree-sitter-typescript` 0.23.2 (user-approved 2026-09-24) so generic call signatures separated only by a newline and `export type * from` parse; upstream 0.23.2 is the latest release and rejected 8 of Hono's 357 eligible files (T46). | `vendor/tree-sitter-typescript/` is a path crate `0.23.2-rivet.1` from tag `v0.23.2` with upstream's MIT license, `RIVET-PATCHES.md`, the committed generated C (CLI 0.24.4, which reproduces upstream's C byte for byte), and `regenerate.sh [--check]`; the build needs no CLI; every file upstream parsed without error (fixture, Hono, extra local corpora) parses to an identical tree; the upstream corpus and new corpus cases pass; `EXTRACTOR_FINGERPRINT` names `ts=0.23.2-rivet.1`; Hono has 0 parse failures and loses no binding. |
...
| [x] | T48 | Prepare held-out task checks and preregistration in small batches. | Corpus, endpoints, sample-size rationale, limits, retry rules, and projected spend are fixed before confirmatory runs. Creating a large corpus is split into batch subtasks. |
...
| [x] | T49 | Execute/analyze the confirmatory benchmark only under an explicit run/spend budget. | Results report passed gates, failure, or inconclusive—not a selected successful-run comparison. Keep this operational job separate from ordinary coding sessions. |
