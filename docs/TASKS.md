...
| Field | Value |
|---|---|
| Last completed task | T46 (pinned real TypeScript project and two-language contract checks, no Rust change: `tests/real/manifest.toml` pins Hono v4.13.9 at `7c3b0df96dbf3e767968ff5afe4c0d999257b8ee` (MIT; 329 `.ts`, 32 `.tsx`, 1 `.d.ts` tracked); `tests/real/fetch.sh` fetches it detached into `$RIVET_REAL_DIR` (default `target/real/`) and verifies the pin; `tests/real/check_real.py` (stdlib, Python 3.9+) checks determinism, full-versus-incremental answers over a seeded 8-step edit script, coverage, two languages in one tree, and budget honesty, all on copies; helper tests `python3 tests/real/test_check_real.py`; results, audit, and timings in `tests/real/RESULTS.md`, known limitations in the TypeScript README); before it, T45 (TypeScript receiver hints as `scoped` bindings, resolver `php-rules-v4;ts-rules-v2`) |
| Next task | T47: compare source estimates against the pilot model's tokenizer and rendered outputs. Follow-ups from T46 for the orchestrator: the pinned TypeScript grammar rejects 8 Hono files (newline-separated generic call signatures, `export type * from`), including `src/context.ts`; bare `.`/`..` specifiers are not relative under the module rule (140 Hono imports); `--no-refresh` queries are slower than refreshed ones on Hono. Parked: T33c, T36a-T36c; TypeScript class-name receivers (`SurveyService.create()`, `Status.Active`) |
| Active task / partial progress | None |
| Blocker | None known |
| Last checks | T46: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo test --workspace` (810 passed, 1 ignored), `check_gold.py` (51 PHP entries verified; 228 TypeScript entries span-verified), `benchmark/runner/test_runner.py` (90 tests), `tests/real/test_check_real.py` (33 tests, also under Python 3.9.6) all pass; `cargo build --release && tests/real/fetch.sh && python3 tests/real/check_real.py` PASS (0 failed): Hono indexes 481 seen / 349 indexed (124 unsupported, 8 parse_error), 2468 symbols, 92303 uses, 9285 bindings (8215 exact, 1070 scoped); determinism byte-identical on 4 trees; full versus `--force` identical after every step (Hono 9904, PHP fixture 1620, TS fixture 2344, mixed 8436 queries per store), and a deliberately stale store FAILs; mixed-tree answers equal single-language ones apart from `index`, 0 cross-language bindings or refs rows; 200 `context` queries respect `--tokens`; audit: 0 wrong of 40 `exact` and 40 `scoped`; Hono cold index 776 ms, no-change refresh 46 ms, force 1156 ms. heldout-01 (T49): outcome `inconclusive`. |
