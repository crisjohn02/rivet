...
PHP namespace lookup uses indexed qualified names without executing Composer/autoload code. TypeScript relative module lookup applies only to `./` and `../` specifiers and the bare specifiers `.` and `..`, joined to the importing file's directory, and checks an explicit ordered candidate set: exact supported path, `.ts`, `.tsx`, `.d.ts`, `/index.ts`, `/index.tsx`, `/index.d.ts`. A specifier that names only a directory, because it ends in `/` or its last segment is `.` or `..` (`.`, `..`, `../..`, `./x/..`, `./`, `./dir/`), checks only that directory's `/index.ts`, `/index.tsx`, `/index.d.ts`, in that order, never the exact path or a `.ts`, `.tsx`, or `.d.ts` file named after the directory, so `..` from `src/a/b.ts` never names `src.ts` (T44a, following TypeScript's own directory handling of such specifiers). Require a unique valid module candidate across the applicable set, never guess among duplicates. Candidates are compared byte for byte with the indexed paths of the snapshot, never the filesystem, so `./Util` does not name `util.ts` on a case-insensitive disk. A candidate that is in the snapshot but not indexed (a parse failure, a size or encoding skip) is not valid, and still counts as a candidate: beside another candidate the import is ambiguous. A specifier that leaves the repository (`..` from a top-level file) or has an empty segment other than one trailing `/` (`.//x`, `./x//`) names no module; the repository root itself is a directory, so `..` from `src/a.ts` checks `index.ts` at the root. Resolve named/default exports only when they identify an explicit local declaration; anonymous default exports remain unresolved. Extensions outside this matrix, including `.js`, `.jsx`, `.mts`, and `.cts`, are not enabled by inference in v0.1.
