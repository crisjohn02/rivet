...
- Receiver hints are recorded for T45 (`this` in a named class, a `const` bound by `new`, one explicit named parameter, field, or variable type); the resolver's rule table binds each language's uses with its own rules over its own declarations only.
...
- Everything else stays unresolved: re-exports, `export *`, an exported import binding (`import { a } from "./x"; export { a }`), path aliases, packages, `require`, a name the module does not export, an unresolved or ambiguous module, and every receiver other than a namespace import (`this.x`, `obj.m()`; T45).
